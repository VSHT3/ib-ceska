...
- **Future developers** — maintaining and extending the site after the current maintainer leaves (see *Succession* below).
...
- **First steps as a developer**
  1. Install Node ≥ 22.12.0, run `npm install`, then `npm run dev`.
  ...
  3. Run `npm run check` before committing; it type-checks the whole project.
