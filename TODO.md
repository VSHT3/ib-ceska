...
- [x] Add body content to subject `.mdx` files (syllabus details, IA criteria)
- [x] Add body content to CAS entries (reflections, evidence)
- [x] Add body content to TOK entries (full essays, discussion prompts)
- [ ] Add real CAS projects the school has done
- [x] Student benefits section sourced from https://www.ibo.org/programmes/diploma-programme/ (university recognition, critical thinking, etc.)
...
- [x] Individual subject detail pages (`/subjects/[slug]`)
- [x] Individual CAS entry detail pages (`/cas/[slug]`)
- [x] Individual TOK entry detail pages (`/tok/[slug]`)
- [x] Individual news article pages (`/news/[slug]`)
- [x] 404 error page
...
- [x] Language switch should keep scroll position (currently jumps to top of page)
...
- [x] `hreflang` alternate links between /en/ and /sk/ pages
- [ ] Full SEO pass (structured data, meta descriptions per page) — later

## UI & UX
- [x] Replace default Astro favicon with school logo
- [ ] Update nav logo with high-quality version when provided (still missing better logo)
- [x] Link to the main school website (gymnaziumceska.sk) — footer and/or nav
- [x] Better footer (richer layout, school-website link, IB logos, legal links)
- [x] Fix scroll overflow color (overscroll background doesn't match design — set `background-color` on `html` / `overscroll-behavior`)
...
## Content features
- [x] RSS/Atom feed for news collection
